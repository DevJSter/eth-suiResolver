...
import "../src/SwapEscrowFactory.sol";

contract DeploySwapFactory is Script {
    function run() external {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        
        vm.startBroadcast(deployerPrivateKey);

        // Deploy the SwapEscrowFactory
        SwapEscrowFactory factory = new SwapEscrowFactory();
        
        console.log("SwapEscrowFactory deployed at:", address(factory));
        ...
        string memory contractAddresses = string(
            abi.encodePacked(
                "SWAP_ESCROW_FACTORY=", vm.toString(address(factory)), "\n"
            )
        );
